...
# MAGIC
# MAGIC Apache Spark 2.0 adds the first version of a new higher-level stream processing API, Structured Streaming. 

# COMMAND ----------

# MAGIC %md ## Sample Data
# MAGIC Data as files in `/databricks-datasets/structured-streaming/events/` which we are going to use to build this appication. 
...
query = (
  streamingCountsDF
    ...
    .trigger(availableNow=True)  # Process all available data once (required for serverless)
    .option("checkpointLocation", "/Volumes/workspace/default/mod7/checkpoint/")  # Required for serverless
    .start()
)
...
query = (
  streamingCountsDF
    ...
    .option("checkpointLocation", "/Volumes/workspace/default/mod7/checkpoint/")
    .start()
)

# COMMAND ----------

dbutils.fs.rm("/Workspace/Users/stephen.giles@utoronto.ca/.checkpoints/streaming-demo/demo/", True) 
