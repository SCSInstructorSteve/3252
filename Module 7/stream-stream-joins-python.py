...
query = impressions.writeStream \
  .format("memory") \
  .queryName("impressions_view") \
  .outputMode("append") \
  .trigger(availableNow=True) \
  .option("checkpointLocation", f"/Volumes/workspace/default/mod7/checkpoint_impressions_{int(time.time())}/") \
  .start()

query.awaitTermination()
display(spark.sql("SELECT * FROM impressions_view"))
...
query = clicks.writeStream \
  .format("memory") \
  .queryName("clicks_view") \
  .outputMode("append") \
  .trigger(availableNow=True) \
  .option("checkpointLocation", f"/Volumes/workspace/default/mod7/checkpoint_clicks_{int(time.time())}/") \
  .start()

query.awaitTermination()
display(spark.sql("SELECT * FROM clicks_view"))
...
joined = impressions.join(clicks, "adId")
...
query = joined.writeStream \
  ...
  .outputMode("append") \
  .trigger(availableNow=True) \
  .option("checkpointLocation", f"/Volumes/workspace/default/mod7/checkpoint_joined_{int(time.time())}/") \
  .start()

query.awaitTermination()
display(spark.sql("SELECT * FROM joined_view"))
...
joined_watermarked = impressionsWithWatermark.join(
  clicksWithWatermark,
  expr(""" 
...
query = joined_watermarked.writeStream \
  .format("memory") \
  .queryName("joined_watermarked_view") \
  .outputMode("append") \
  .trigger(availableNow=True) \
  .option("checkpointLocation", f"/Volumes/workspace/default/mod7/checkpoint_watermarked_{int(time.time())}/") \
  .start()

query.awaitTermination()
display(spark.sql("SELECT * FROM joined_watermarked_view"))
...
joined_left_outer = impressionsWithWatermark.join(
  clicksWithWatermark,
  expr(""" 
    clickAdId = impressionAdId AND 
    clickTime >= impressionTime AND 
...
query = joined_left_outer.writeStream \
  .format("memory") \
  .queryName("joined_left_outer_view") \
  .outputMode("append") \
  .trigger(availableNow=True) \
  .option("checkpointLocation", f"/Volumes/workspace/default/mod7/checkpoint_left_outer_{int(time.time())}/") \
  .start()

query.awaitTermination()
display(spark.sql("SELECT * FROM joined_left_outer_view"))
